...
The Redfield ratio refers to the carbon / nitrogen / phosphorous ratio found in phytoplankton across the oceans, which is 106:16:1 (or simply, 16:1). The ratio is considered an important baseline for healthy ocean life. Scientists can use the ratio to track changes in the ocean by assessing any variances in this ratio.\
\
...
```{r}
bg_chem <- read_csv(file = "../Data/RAW/BGchem2008data.csv") 
```
...
The plot above displays the relationship between phosphate and total nitrogen concentration in ocean water samples. The concentration shows a linear correlation between phosphate and total nitrogen. There is a cluster of points that show when total nitrogen concentration is about 13, phosphate ranges from 0.8 to 1.1.
